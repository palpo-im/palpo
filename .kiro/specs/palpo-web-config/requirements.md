...
当前用户管理实现与 Synapse Admin API 存在功能差距，需要补充以下能力以满足生产环境需求。
...
1. 扩展 `User` 和 `UserDetails` 数据结构，添加缺失字段
2. 实现 `threepid_repository.rs` 管理第三方身份
3. 实现 `external_ids` 支持 SSO 场景
4. 添加用户级速率限制配置端点
5. 实现设备管理、连接管理、推送器管理相关端点
6. 添加用户房间成员资格查询功能
7. 实现批量用户操作（CSV 导入、批量通知）

#### 优先级

- P0: threepids、external_ids、数据擦除（Synapse API 对齐）
- P1: 设备管理、用户房间列表（基础关联功能）
- P2: 速率限制、实验功能、账户数据（高级功能）
- P3: 批量操作、连接管理、推送器管理（辅助功能）