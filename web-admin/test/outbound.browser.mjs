...
f.palpo.fetch = (url, options) => { assert.ok(!new URL(url).pathname.startsWith('/api/fleet/v1'), 'Outbound UI must never call Hagency'); return f.fetch(url, options); };
...
try {
  ...
  const downloadPromise = page.waitForEvent('download'); await page.getByRole('button', { name: 'Download Hagency configuration', exact: true }).click();
  ...
  const machine = (path, body) => http(`/api/fleet/v2/${fleet.id}${path}`, { body, headers: { Authorization: `Bearer ${credentials.transport.token}`, 'X-Hagency-Generation': '1' } });
  ...
  await page.getByText('Verification event queued for Hagency. Receipt will be confirmed over its outbound connection.', { exact: true }).waitFor();
  ...
  await page.locator('#requests').getByText('Stored in Palpo. Waiting for Hagency to receive this request; delivery does not approve or allocate an Agent.', { exact: true }).waitFor();
