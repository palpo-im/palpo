...
-- Dynamic presence EDUs are rebuilt after a failed federation request. Keep the
-- observation time that belongs to each stored presence row so time-derived fields such
-- as `last_active_ago` are byte-stable across those retries.
