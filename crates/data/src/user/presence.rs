...
///
/// Takes the presence stream lock like every other publication, so sync cursors cannot
/// skip it, and refreshes `updated_at` so rebuilt federation EDUs observe it at this time.
        ...
        .transaction::<_, crate::DataError, _>(async |conn| {
            ...
            diesel::insert_into(user_presences::table)
                .values((
                    ...
                    user_presences::updated_at.eq(now),
                ))
                .on_conflict(user_presences::user_id)
                .do_update()
                .set((
