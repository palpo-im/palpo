...
pub struct AuthMetadtaResBody {
    /// The authorization server metadata as defined in [RFC 8414].
    ///
    /// [RFC 8414]: https://datatracker.ietf.org/doc/html/rfc8414
    #[serde(flatten)]
    #[salvo(schema(value_type = Object, additional_properties = true))]
    pub metadata: RawJson<AuthorizationServerMetadata>,
}

impl AuthMetadtaResBody {
    /// Creates a new `Response` with the given `HomeServerInfo`.
    pub fn new(metadata: RawJson<AuthorizationServerMetadata>) -> Self {
        Self { metadata }
    }
}
    ...
    fn validate_urls_inner(
        ...
        let optional_urls = &[
            self.registration_endpoint
                .as_ref()
                .map(|string| ("registration_endpoint", string)),
            #[cfg(feature = "unstable-msc4191")]
...
pub enum AccountManagementAction {
    ...
    ///
    /// [RFC 7636]: https://datatracker.ietf.org/doc/html/rfc7636
    #[palpo_enum(rename = "org.matrix.profile")]
    Profile,

    /// The user wishes to view a list of their sessions.
    #[palpo_enum(rename = "org.matrix.sessions_list")]
    SessionsList,

    /// The user wishes to view the details of a specific session.
    #[palpo_enum(rename = "org.matrix.session_view")]
    SessionView,

    /// The user wishes to end/logout a specific session.
    #[palpo_enum(rename = "org.matrix.session_end")]
    SessionEnd,

    /// The user wishes to deactivate their account.
    #[palpo_enum(rename = "org.matrix.account_deactivate")]
    AccountDeactivate,

    /// The user wishes to reset their cross-signing keys.
    #[palpo_enum(rename = "org.matrix.cross_signing_reset")]
    CrossSigningReset,
