...
pub enum AuthType {
    ...
    #[palpo_enum(rename = "m.login.password")]
    Password,

    /// Google ReCaptcha 2.0 authentication (`m.login.recaptcha`).
    #[palpo_enum(rename = "m.login.recaptcha")]
    ReCaptcha,
    ...
    #[palpo_enum(rename = "m.login.msisdn")]
    Msisdn,

    /// SSO-based authentication (`m.login.sso`).
    #[palpo_enum(rename = "m.login.sso")]
    Sso,

    /// Dummy authentication (`m.login.dummy`).
    #[palpo_enum(rename = "m.login.dummy")]
    Dummy,
...
impl fmt::Display for UiaaInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "User-Interactive Authentication required.")
    }
}
impl StdError for UiaaInfo {}
...
//                 .body(crate::serde::json_to_buf(&authentication_info)?)
//                 .map_err(Into::into),
