...
impl RtcNotificationEventContent {
    ...
    /// `let duration_ring =
    /// UnixMillis::now().saturated_sub(content.expiration_ts(event.
    ...
    pub fn expiration_ts(
        ...
        let start_ts = if use_origin_server_ts {
            origin_server_ts.get()
        } else {
            self.sender_ts.get()
        };
        UnixMillis(start_ts.saturating_add(self.lifetime.as_millis() as u64))
    }
}
...
mod tests {
    ...
    fn notification_event_serialization() {
        let mut content = RtcNotificationEventContent::new(
            UnixMillis(1_752_583_130_365),
            Duration::from_millis(30_000),
            NotificationType::Ring,
        );
        content.mentions = Some(Mentions::with_room_mention());
        content.relates_to = Some(crate::events::relation::Reference::new(owned_event_id!(
            "$m:ex"
        )));
    ...
    fn expiration_ts_computation() {
        ...
        let origin_server_ts = UnixMillis(120_000);
        assert_eq!(
        ...
        let origin_server_ts = UnixMillis(200_000);
        assert_eq!(
            content.expiration_ts(origin_server_ts, None),
            UnixMillis(230_000)
        );
        ...
        let origin_server_ts = UnixMillis(50_000);
        assert_eq!(
            content.expiration_ts(origin_server_ts, None),
            UnixMillis(80_000)
        );
        ...
        let origin_server_ts = UnixMillis(130_200);
        assert_eq!(
        ...
        let origin_server_ts = UnixMillis(100_300);
        assert_eq!(
            content.expiration_ts(origin_server_ts, Some(100)),
            UnixMillis(130_365)
        );
