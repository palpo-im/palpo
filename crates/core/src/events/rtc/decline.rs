...
#[palpo_event(type = "org.matrix.msc4310.rtc.decline", alias = "m.rtc.decline", kind = MessageLike)]
...
impl RtcDeclineEventContent {
    ...
    pub fn new<E: Into<crate::OwnedEventId>>(notification_event_id: E) -> Self {
        Self {
            relates_to: Reference::new(notification_event_id.into()),
        }
    }
}
...
mod tests {
    ...
    fn decline_event_deserialization() {
        ...
        assert_eq!(
            decline_event.content.relates_to.event_id,
            "$abc:example.org"
        );
    }
}
