...
- [ ] 多处 Matrix 协议缺口仍以明确错误返回，需要按协议功能专项补齐。
  - [x] dehydrated device GET/PUT/DELETE 存储闭环已接入，不再对基本设备脱水管理返回未实现。
  - [ ] dehydrated device to-device event delivery 仍未实现。
  - [ ] third-party invite exchange 仍未实现。
  - [ ] 3PID bind / identity server 绑定流程仍未实现。
