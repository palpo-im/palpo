...
- `bridge.app_id` and `bridge.app_secret` (from Feishu app)
...
export MATRIX_BRIDGE_FEISHU_DB_URI="sqlite:/data/matrix-feishu.db"
export MATRIX_BRIDGE_FEISHU_AS_TOKEN="your_as_token"
export MATRIX_BRIDGE_FEISHU_HS_TOKEN="your_hs_token"
export MATRIX_BRIDGE_FEISHU_BRIDGE_APP_ID="your_app_id"
export MATRIX_BRIDGE_FEISHU_BRIDGE_APP_SECRET="your_app_secret"
```
...
- Port 8081: Feishu webhook endpoint (public)
...
```bash
export MATRIX_BRIDGE_FEISHU_PROVISIONING_READ_TOKEN="read_token"
export MATRIX_BRIDGE_FEISHU_PROVISIONING_WRITE_TOKEN="write_token"
export MATRIX_BRIDGE_FEISHU_PROVISIONING_DELETE_TOKEN="delete_token"
```
...
docker compose down
docker compose up -d
docker compose logs -f matrix-bridge-feishu
```
...
1. **Webhook signature failed**: Verify `bridge.listen_secret` matches your Feishu app configuration
2. **Permission denied**: Check Feishu app scopes for message send/read and file/image APIs
3. **Messages not bridging**: Verify event subscriptions are enabled in Feishu app
...
4. Review logs: `docker compose logs matrix-bridge-feishu | tail -100`
